...
export const useSystemSettings = () => {
  ...
  const loadAllSettings = async () => {
    ...
    try {
      ...
      // Fetch preferences - if doesn't exist, create it on-demand
      ...
      // If no preferences row, create one with defaults
      if (!prefs) {
        try {
          await (supabase
            .from('user_preferences' as any)
            ...
            .select() as any);
          console.log('Auto-created user_preferences row');
        } catch (insertError) {
          console.log('Preferences row already exists or creation skipped:', insertError);
        }
        // Use default values for UI
  ...
  const savePreferences = async () => {
    ...
    try {
      ...
      // If no rows were updated, the preferences row doesn't exist yet - create it
