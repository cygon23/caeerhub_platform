...
export const ThemeProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const loadUserTheme = async () => {
    ...
    try {
      ...
      // If user_preferences row doesn't exist yet (new user), use defaults
      if (error) {
        console.log("No preferences found for new user (expected):", error);
        applyTheme("light", "blue");
        return;
      }
      ...
      if (data) {
        ...
        // No preferences row yet - use defaults (will be created later)
        applyTheme("light", "blue");
      }
    } catch (error) {
      console.error("Error loading theme:", error);
      // Always fall back to defaults on any error
      applyTheme("light", "blue");
    }
  };
