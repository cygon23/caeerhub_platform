...
export default function AcademicDashboardTab({ profile, onNavigate }: Props) {
  ...
  const loadDashboardData = async () => {
    try {
      const [assignData, groupData, schedData] = await Promise.all([
        ...
        <CardContent className="p-5">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <h3 className="font-semibold mb-1">Your Study Focus</h3>
              <p className="text-sm text-muted-foreground">
                ...
                {profile.specific_struggles && (
                  <span> Focus area: {profile.specific_struggles}</span>
                )}
              </p>
            </div>
          </div>
          ...
          <CardContent className="space-y-3">
            ...
            {upcomingAssignments.length === 0 && overdueAssignments.length === 0 ? (
              <div className="text-center py-6 text-muted-foreground text-sm">
                ...
                No assignments yet.
                <Button variant="link" size="sm" onClick={() => onNavigate('assignments')}>Create one</Button>
              </div>
            ) : (
          ...
          <CardContent className="space-y-2">
            {todaySchedules.length === 0 ? (
              <div className="text-center py-6 text-muted-foreground text-sm">
                ...
                <Button variant="link" size="sm" onClick={() => onNavigate('schedule')}>Add one</Button>
              </div>
            ) : (
