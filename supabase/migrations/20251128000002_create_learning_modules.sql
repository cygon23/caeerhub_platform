...
CREATE TABLE IF NOT EXISTS public.learning_modules (
  ...
  title TEXT NOT NULL,
  description TEXT,
  content TEXT, -- Main content/article text
  thumbnail_url TEXT,
  ...
  -- Content URLs
  video_url TEXT, -- YouTube, Vimeo, or uploaded video URL
  pdf_url TEXT, -- PDF document URL
  external_link TEXT, -- External resource link

  -- Metadata
  prerequisites TEXT[], -- Array of prerequisite module titles or skills
  learning_objectives TEXT[], -- What users will learn
  target_audience TEXT[] DEFAULT ARRAY['youth'], -- youth, mentor, student, all
...
-- Create user_module_progress table for tracking
...
-- Create user_learning_preferences table for AI personalization
CREATE TABLE IF NOT EXISTS public.user_learning_preferences (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL UNIQUE REFERENCES auth.users(id) ON DELETE CASCADE,

  -- User preferences
  selected_topics TEXT[], -- Topics user wants to learn
  learning_goals TEXT[], -- User's learning goals
  skill_gaps TEXT[], -- Skills user wants to improve
  preferred_difficulty TEXT DEFAULT 'beginner' CHECK (preferred_difficulty IN ('beginner', 'intermediate', 'advanced')),
  preferred_categories TEXT[], -- Preferred module categories
...
CREATE INDEX IF NOT EXISTS idx_learning_modules_created_at ON public.learning_modules(created_at DESC);
...
-- Function to increment module views
...
-- Function to track module enrollment
...
BEGIN
  ...
  VALUES (auth.uid(), p_module_id, 'in_progress', NOW(), NOW())
